...
namespace Moka
{
  ...
  private:
    template<typename... Ts, size_t... ind, typename Func>
    void describe_many_impl(std::string name, const std::vector<std::string>& placeholders, Func fn, std::index_sequence<ind...>) { // fn = void<T>(Context&)
      (
        (
    ...
    template<typename... Ts, size_t... ind, typename Func>
    void describe_for_impl(std::string name, const std::tuple<std::pair<Ts, std::string>...>& params, Func fn, std::index_sequence<ind...>) { // fn = void<T>(Context&)
      (
  ...
  public:
    ...
    void describe_many(std::string name, const std::initializer_list<std::string>& placeholders, Func fn) { // fn = void<T>(Context&)
      describe_many_impl<Ts...>(name, placeholders, fn, std::index_sequence_for<Ts...>{});
    }

    template<typename... Ts, typename Func>
    void describe_for(std::string name, const std::tuple<std::pair<Ts, std::string>...>& params, Func fn) { // fn = void<T>(Context&)
